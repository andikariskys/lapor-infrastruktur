    ...
    <form action="{{ url('/laporan/' . $report['id']) }}" method="POST" class="space-y-6 lg:space-y-8">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-blue-900 mb-3">{{ $report['category']['name'] ??
                        'Kategori Umum' }}</h1>
            ...
            ? 'https://www.google.com/maps?q=' . rawurlencode($latitude . ',' . $longitude)
            : null;
            @endphp
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                    ...
                    <a href="{{ $mapsUrl }}" target="_blank" rel="noopener noreferrer">
                        <p class="text-sm font-semibold text-gray-800 mb-1 group-hover:text-blue-700">Koordinat:
                            {{ $latitude }}, {{ $longitude }}</p>
                        <p class="text-xs text-gray-500 group-hover:text-blue-600">Klik untuk membuka lokasi di Google
                            Maps</p>
                    </a>
                    @else
                    <p class="text-sm font-medium text-gray-700 mb-1">Koordinat: {{ $latitude }},
                        {{ $longitude }}</p>
                ...
                @if($mapsUrl)
                <a href="{{ $mapsUrl }}" target="_blank" rel="noopener noreferrer"
                    class="bg-green-50 rounded-2xl border border-green-100 relative overflow-hidden h-32 flex items-center justify-center transition-all duration-200 hover:border-green-300 hover:shadow-sm group">
                    <iframe
                        src="https://www.openstreetmap.org/export/embed.html?layer=mapnik&marker={{ rawurlencode($latitude . ',' . $longitude) }}&bbox={{ rawurlencode(($longitude - 0.008) . ',' . ($latitude - 0.008) . ',' . ($longitude + 0.008) . ',' . ($latitude + 0.008)) }}"
                        class="absolute inset-0 w-full h-full border-0 opacity-90" loading="lazy"
                        referrerpolicy="no-referrer-when-downgrade" title="Peta lokasi laporan"></iframe>
                    <div class="absolute inset-0 bg-white/15 group-hover:bg-white/10 transition-colors"></div>
                    <div
                        class="relative bg-white/95 px-3 py-2 rounded-xl shadow-sm border border-green-200 flex items-center gap-2 group-hover:border-green-300">
            ...
            <div class="mt-8 pt-8 border-t border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                    ...
                    <div class="bg-gray-100 rounded-2xl border border-gray-200 relative overflow-hidden h-40 group cursor-pointer"
                        onclick="openResolutionPhotoModal()">
                        <img src="{{ config('app.backend_url') . $report['completion_photo'] }}" alt="Bukti Perbaikan"
                            class="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                        <div
                            class="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                            <span
                                class="text-white text-xs font-bold bg-black/50 px-3 py-1.5 rounded-full backdrop-blur-sm">Lihat
                                Foto Bukti</span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="md:col-span-2 space-y-4">
                        ...
                        <div class="bg-blue-50/50 rounded-2xl border border-blue-100 p-5">
                            <p class="text-[10px] uppercase font-bold text-blue-600 tracking-wider mb-2">Catatan
                                Penugasan (Admin)</p>
                            <p class="text-sm text-gray-700 leading-relaxed font-semibold italic">
                                "{{ $currentAssignment['note'] }}"
                            </p>
                            <div class="mt-4 flex items-center gap-2 text-xs text-gray-500">
                                <span class="font-bold text-gray-700">{{ $currentAssignment['officer']['name'] ??
                                    'Petugas' }}</span>
                                <span>•</span>
                                <span>Ditugaskan pada {{ \Carbon\Carbon::parse($currentAssignment['assigned_at'] ??
                                    now())->format('Y-m-d H:i') }} WIB</span>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="bg-green-50/50 rounded-2xl border border-green-100 p-5">
                            <p class="text-[10px] uppercase font-bold text-green-600 tracking-wider mb-2">Balasan
                                Progres (Petugas)</p>
                            <p class="text-sm text-gray-700 leading-relaxed font-semibold italic">
                            ...
                            <div class="mt-4 flex items-center gap-2 text-xs text-gray-500">
                                <span class="font-bold text-gray-700">{{ $currentAssignment['officer']['name'] ??
                                    'Petugas' }}</span>
                                <span>•</span>
                                <span>Diupdate pada {{ \Carbon\Carbon::parse($report['updated_at'] ??
                                    now())->format('Y-m-d H:i') }} WIB</span>
                            </div>
                        </div>
                        @endif
            ...
            <div class="mt-8 pt-8 border-t border-gray-100">
                <div class="flex items-center gap-2 text-orange-600 font-bold mb-4">
                    <svg class="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M11.48 3.499c.158-.326.608-.326.765 0l1.597 3.238 3.57 1.055c.362.107.507.553.243.812l-2.585 2.531.616 3.557c.063.364-.32.643-.642.47L12 10.978l-3.197 1.674c-.322.172-.705-.107-.642-.47l.616-3.557-2.585-2.531c-.264-.259-.119-.705.243-.812l3.57-1.055 1.597-3.238Z" />
                    </svg>
                ...
                <div class="bg-orange-50/50 rounded-2xl border border-orange-100 p-5">
                    <div class="flex items-center gap-1.5 mb-3">
                        @for($i = 1; $i <= 5; $i++) <svg
                            class="w-5 h-5 {{ $i <= $currentFeedback['rating'] ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200' }}"
                            viewBox="0 0 20 20">
                            <path
                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                            @endfor
                            <span class="ml-2 text-sm font-extrabold text-orange-700">({{ $currentFeedback['rating'] }}
                                / 5)</span>
                    </div>
                    <p class="text-sm text-gray-700 leading-relaxed font-semibold italic">
                    ...
                    <div class="mt-4 flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <span>•</span>
                        <span>Dikirim pada {{ \Carbon\Carbon::parse($currentFeedback['created_at'] ??
                            now())->format('Y-m-d H:i') }} WIB</span>
                    </div>
                </div>
            </div>
        ...
        <div id="verification-section"
            class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 {{ in_array($report['status'], ['in_progress', 'resolved']) ? 'opacity-50 pointer-events-none' : '' }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label class="relative cursor-pointer group">
                    <input type="radio" name="status" value="verified" class="peer hidden" {{
                        $report['status']=='verified' ? 'checked' : '' }}>
                    <div
                ...
                <label class="relative cursor-pointer group">
                    <input type="radio" name="status" value="spam" class="peer hidden" {{ $report['status']=='spam'
                        ? 'checked' : '' }}>
                    <div
        ...
        <div id="assignment-section"
            class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 {{ $report['status'] !== 'verified' ? 'opacity-50 pointer-events-none' : '' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="select-institution" name="institution_id" onchange="filterOfficers()"
                        ...
                        <option value="{{ $inst['id'] }}" {{ ($currentInstitutionId==$inst['id']) ? 'selected' : '' }}>
                            {{ $inst['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="select-officer" name="officer_id" disabled
                        ...
                        <option value="{{ $off['id'] }}" data-institution="{{ $off['institution_id'] }}" {{
                            ($currentOfficerId==$off['id']) ? 'selected' : '' }}>{{ $off['name'] }}
                            ({{ $off['email'] }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mt-6">
                <label class="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-wider">Pesan untuk
                    Petugas</label>
                <textarea id="note-textarea" name="note" rows="3" disabled
                    placeholder="Tulis catatan atau instruksi untuk petugas pelaksana..."
...
<div id="resolution-photo-modal"
    ...
    <div class="w-full h-full p-4 sm:p-12 flex items-center justify-center pt-24">
        <img src="{{ config('app.backend_url') . $report['completion_photo'] }}" alt="Bukti Perbaikan"
            class="max-h-full max-w-full object-contain rounded-2xl shadow-2xl ring-1 ring-white/10">
    </div>
</div>
@endif
